...
function getParentOfTag(element,tagName)
{
    ...
    while (tag.node().tagName != tagName)
    {
...
IconMaker.prototype.init = function()
{
    ...
    this.yScale = d3.scale.linear() //d3.scaleLinear() in v4 
                    .domain([0,100])
                    .range([Math.max(0,(height-width)/2), Math.min(height,(height + width)/2)])  
    this.xScale = d3.scale.linear() //d3.scaleLinear() in v4
                    .domain([0,100])
...
IconMaker.prototype.add = function(d, node)
{
    ...
    { 
        if (d.shape == "line"){
           ...
           var arc = d3.svg.arc() //d3.arc() in v4 
...
IconMaker.prototype.iconData = function(name)
{
    ...
    switch (name) 
    {
      ...
      case "progress":
        ...
        var percentSuccess  = (shapeData.hasOwnProperty("pct-success"))? shapeData["pct-success"]: 0;
        var percentFailure  = (shapeData.hasOwnProperty("pct-failure"))? shapeData["pct-failure"]: 0;
        var percentWarning  = (shapeData.hasOwnProperty("pct-warning"))? shapeData["pct-warning"]: 0;
        ...
        {  
           data.push({"shape":"circle","cx":50,"cy":50,"r":46,"stroke":"#666","stroke-width":4,"fill":"none"});
           if (percentSuccess > 0) 
               data.push({"shape":"arc","cx":50,"cy":50,"innerRadius":0,"outerRadius":40,"fill":this.green ,"startAngle":0,"endAngle":percentSuccess});
           if (percentFailure > 0) 
               data.push({"shape":"arc","cx":50,"cy":50,"innerRadius":0,"outerRadius":40,"fill":this.red   ,"startAngle":percentSuccess,"endAngle":percentSuccess + percentFailure});
           if (percentWarning > 0) 
               data.push({"shape":"arc","cx":50,"cy":50,"innerRadius":0,"outerRadius":40,"fill":this.yellow,"startAngle":percentSuccess + percentFailure,"endAngle":percentSuccess + percentFailure + percentWarning});
           data.push({"shape":"circle","cx":50,"cy":50,"r":40,"fill":"#001f3f","class":"pulse"});
        }
        break;
      default:
